...
function BSNFE_wh_phoneValidateCheckoutFields() {
    $billing_options_typefac = filter_input(INPUT_POST, 'billing_options_typefac', FILTER_SANITIZE_STRING);
    ...
    $billing_options_razonsocial = filter_input(INPUT_POST, 'billing_options_razonsocial', FILTER_SANITIZE_STRING);
    $billing_options_corre = filter_input(INPUT_POST, 'billing_options_corre', FILTER_SANITIZE_STRING);
    // Validar si va a emitir
    if($billing_options_typefac == "01")
    {
        ...
        if($billing_options_iTipoRec == "01" || $billing_options_iTipoRec == "03"){
            if(empty($billing_options_razonsocial)){
                wc_add_notice('La <strong>razón social</strong> es requerida.', 'error');
            }
            if (empty($billing_options_dv)) {
                wc_add_notice('El <strong>dígito verificador</strong> es requerido.', 'error');
            }
        }
        if($billing_options_iTipoRec != "02"){
